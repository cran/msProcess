...
## use RSQLite to read in data from SQLite database
if(!require(RSQLite)) stop("RSQLite is required but not available")
dbname <- ifelse(is.R(), paste(system.file("data", package="msProcess"), "qcset.db", sep="/"), "qcset.db")
...
qcset <- list(mz=mz[,1], intensity=as.matrix(intensity), type=as.factor(type[,1]))
attr(qcset, "class") <- "msSet"
attr(qcset, "data.name") <- "qcset"
dbDisconnect(con)
